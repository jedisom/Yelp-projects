...
#Get document lengths based on terms found before filtering out terms that are the same
X[,"doc.lengths"] <- as.data.frame(as.matrix(rollup(dtm, 2, na.rm=TRUE, FUN = sum)))

#only include terms that are different between pass and fail for future analysis
...
zero.topic.count <- 10
one.topic.count <- 10
topics0 <- LDA(dtm0, zero.topic.count, method = "Gibbs")
topics1 <- LDA(dtm1, one.topic.count, method = "Gibbs")
...
Xs <- X[,c("review.count", "avg rating", "F1", topic0names, topic1names)]
...
for (i in regConstants){
      #Liblinear does logistic regression with regularization AND cross validation all in one function!
      temp <- LiblineaR(data=as.matrix(Xs[tRows, ]), target=tLabels, cost=i, cross=10) 
      print(paste(i, cat("\t"), temp))
}
#here I manually picked the cost value that seemed to maximize the prediction accuracy
logRegModel <- LiblineaR(data = as.matrix(Xs[tRows, ]), target=tLabels, cost=3)
...
#train svm model
Xs <- X[,c("review.count", "avg rating", "F1", topic0names, topic1names, "logReg")]
c <- heuristicC(as.matrix(Xs[tRows, ]))
regConstants <- c(.0000001, .0000003, .000001, .000003, .00001, .00003, .0001, .0003, .001, .003, 
                  .01, .03, .1, .3, 1, 3, 10, 30, 100, 300)
set.seed(1)
for (i in regConstants){
      ...
      #Liblinear does SVM with regularization AND cross validation all in one function!
